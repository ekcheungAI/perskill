...
import {
  ...
  Sparkles, Target, TrendingUp, AlertCircle, Download
} from "lucide-react";
...
function buildNuwaSkillExport(stack: StackRecommendation, userProfile: UserProfile): string {
  const stackNames = stack.personas.map((p) => p.name).join(" + ");
  const topPersona = stack.personas[0];
  ...
  return [
    `---
name: composite-stack-${stack.personas.map((p) => p.id).join("-")}
description: |
  Composite AI Skill combining ${stackNames}.
  Generated by Perskill Persona Match on ${new Date().toISOString().split("T")[0]}.
  User profile: ${userProfile.name || "anonymous"}, ${userProfile.role || "professional"}.
  Stack synergy score: ${stack.synergyScore}%.
---

# ${stackNames} · Composite Thinking Partner

> "${combinedIdentity}"

## Who This Skill Is For

**User Profile**: ${userProfile.name || "The user"} — ${userProfile.role || "professional"}
- **Strong in**: ${userStrengths}
- **Gaps to fill**: ${userGaps}

---

## Composite Stack Identity

This skill combines ${stackNames} into a single AI persona calibrated to complement your working style.

**Activation trigger phrases**:
- "${topPersona.name} + ${secondPersona?.name ? secondPersona.name : topPersona.name} mode"
- "Analyze with composite thinking partner"
- "${topPersona.name.split(" ")[0]} lens" or "${secondPersona?.name.split(" ")[0] ?? topPersona.name.split(" ")[0]} lens"

---

## Role-Playing Rules

**Once this skill is activated, respond directly as the composite identity of ${stackNames}.**

- Use first person ("I") not third person ("${stackNames} would think...")
- Use Persona A — ${topPersona.name}'s tone: ${topPersona.communicationStyle || "direct, opinionated"}
${secondPersona ? `- Use Persona B — ${secondPersona.name}'s frameworks: ${secondPersona.communicationStyle || "strategic lens"}\n- When the user is in their own strength areas, use A's intuition; when filling the user's gap areas, use B's frameworks` : ""}
...
**Exiting role**: Say "exit" or "return to normal mode" to restore normal mode.
...
### Step 1: Classify
Is this question primarily about: (a) something in the user's gap areas → use ${secondPersona?.name.split(" ")[0] ?? topPersona.name.split(" ")[0]} frameworks; (b) something in the user's strength areas → use ${topPersona.name.split(" ")[0]} to add depth; (c) complex strategic question → blend both frameworks.

### Step 2: Research
For fact-dependent questions, use web search to get real data before answering. Don't guess numbers, dates, or facts.

### Step 3: Respond
Lead with the decisive answer. Then explain using ${topPersona.thinkingFrameworks?.[0]?.name ?? "first principles"}. When the user's gap areas are involved, explicitly note: "From the ${secondPersona?.name.split(" ")[0] ?? topPersona.name.split(" ")[0]} perspective..."

---

## Core Frameworks

${allFrameworks.map((f, i) => `### ${i + 1}. ${f.name}\n${f.description || ""}\n*How to apply*: ${f.howToApply || "See persona detail page"}\n`).join("\n")}

---

## Decision Heuristics

${allHeuristics.map((h, i) => `### ${i + 1}. ${h.name}\nWhen: ${h.scenario}\nExample: ${h.example}\n`).join("\n")}
...
${allValues.length > 0
  ? allValues.slice(0, 6).map((v) => `- **${v.value}**: ${v.description}`).join("\n")
  : `• ${topPersona.personalityTraits?.slice(0, 3).join(" · ") ?? "Results, speed, precision"}`}
...
${allAntiPatterns.length > 0
  ? allAntiPatterns.map((ap) => `❌ **${ap.behavior}**: ${ap.reason}`).join("\n")
  : `❌ **Incremental thinking**: Avoid 10% improvements — always ask: what would a 10x solution look like?\n❌ **Consensus-seeking**: When action is needed, act. Consensus is the enemy of speed.`}

---

## Honest Boundaries

⚠️ This composite skill has the combined limitations of both component personas.

${allBoundaries.length > 0
  ? allBoundaries.map((b, i) => `${i + 1}. **${b.limitation}**: ${b.explanation} → Usage note: ${b.implication}`).join("\n")
  : `1. **Technology judgment**: ${topPersona.name.split(" ")[0]} approaches tech from a business/engineering lens. For consumer software specifics, verify with current data.
2. **Political judgment**: Both personas have limited political judgment. Defer to current events data.
3. **Information cutoff**: Both personas' knowledge has a cutoff date. Always verify current facts.`}

---

## Composite System Prompts

### Full ${topPersona.name} Prompt
${topPersona.aiPersonaPrompt}

${secondPersona ? `### Full ${secondPersona.name} Prompt
${secondPersona.aiPersonaPrompt}` : ""}

---

*Generated by Perskill · ${new Date().toISOString().split("T")[0]} · Composite stack score: ${stack.synergyScore}%*`,
  ].join("\n");
}
...
function StepResults({
  ...
  const handleExportNuwa = () => {
    const nuwaFormat = buildNuwaSkillExport(stack, userProfile);
    const blob = new Blob([nuwaFormat], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    const namePart = stack.personas.map((p) => p.name.split(" ")[0].toLowerCase()).join("-");
    a.download = `composite-stack-${namePart}-${Date.now()}.md`;
    a.click();
    URL.revokeObjectURL(url);
    toast.success("Nuwa Skill exported! Download the .md file and paste into Claude Code.");
  };
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-gray-900 rounded-2xl p-5 text-white">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={handleExportNuwa}
                className="w-full flex items-center justify-center gap-2 py-2 bg-amber-500 text-gray-900 rounded-xl text-[12px] font-bold hover:bg-amber-400 transition-colors"
                style={{ fontFamily: "Inter, sans-serif" }}
              >
                <Download size={13} />
          ...
          <div className="flex items-center gap-3">
            <Link href="/">
              <div className="flex items-center gap-2 text-gray-900 hover:text-gray-600 transition-colors">
                <ArrowLeft size={14} />
                <span className="text-[12px] font-medium" style={{ fontFamily: "Inter, sans-serif" }}>Perskill</span>
              </div>
            </Link>
