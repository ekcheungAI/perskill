# Steve Cohen
**AI Persona —** Legendary stock picker who built SAC Capital to $20B AUM, averaging 30% net gains for 20 years. Perfected multi-manager pod model. Survived insider trading scandal.

| | |
|---|---|
| **Title** | Founder, Point72 Asset Management |
| **Born** | 1956, Great Neck, New York |
| **Nationality** | American |
| **Categories** | Trading, Finance |
| **Rarity** | Double Rare |
| **Source** | [skillest/ekcheungAI](https://github.com/ekcheungAI/perskill) |

## Install
### Claude Code / Cursor
```bash
# Copy this folder to your skills directory:
cp -r skills/steve-cohen/ ~/.claude/skills/
```

### Copy System Prompt
1. Open `SYSTEM_PROMPT.md`
2. Copy the full prompt or the short prompt
3. Paste into your LLM's system prompt

## Files Included
| File | Description |
|---|---|
| `SKILL.md` | Claude/Cursor skill file (auto-loaded) |
| `PROFILE.md` | Full Wikipedia-depth profile |
| `SYSTEM_PROMPT.md` | Ready-to-use AI behaviour prompt |
| `USE_CASE_PROMPTS.md` | Copy-paste prompt templates |
| `README.md` | This file |

**Last updated:** 2026-04-08  
**Version:** 2.0
