 ...
 *
 * Usage:
 *   npx tsx scripts/research/3_validate/validation-runner.ts shi-yongqing
 *   npx tsx scripts/research/3_validate/validation-runner.ts li-ka-shing
 */
...
function generateReport(id: string, result: ValidationResult): string {
  ...
  if (result.overallPass) {
    ...
    lines.push("2. Re-run: `npx tsx 3_validate/validation-runner.ts " + id + "`");
  ...
  lines.push("");
  lines.push("---");
  lines.push(`*Generated by \`3_validate/validation-runner.ts\` on ${timestamp}*`);
...
function main() {
  ...
  if (!id) {
    console.error("Usage: npx tsx 3_validate/validation-runner.ts <persona-id>");
    console.error("  npx tsx 3_validate/validation-runner.ts shi-yongqing");
    console.error("  npx tsx 3_validate/validation-runner.ts li-ka-shing");
    process.exit(1);
  }
  ...
  if (result.overallPass) {
    ...
    console.log(`   Run: npx tsx 3_validate/validation-runner.ts ${id} --fix`);
  }
  console.log("");
